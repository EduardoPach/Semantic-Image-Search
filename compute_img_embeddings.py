...
def main(
    ...
    for batch_idx, batch in enumerate(loop):
        ...
        name_prefix = embeddings_temp_dir / f"{batch_idx:05d}"
    ...
    if os.path.exists(index_checkpoint):
        url_file_path = embeddings_dir / (index_checkpoint.split(".")[0]+'.parquet')
        curr_img_path = pd.read_csv(url_file_path)
        img_path = pd.concat([curr_img_path, img_path]).reset_index(drop=True)

    img_path.to_csv(url_file_path, index=False)
